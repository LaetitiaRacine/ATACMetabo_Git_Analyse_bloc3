...
# Working directory
the_dir = "/home/rparmentier/Documents/ATACseqMetabo1-2/"
setwd(the_dir)

conditions_list = list("Xvivo"= list("00h" = c("LAS","LBK", "JLL")),
                       "MP" = list("03h" = c("JLN","LAU","LBM"),
                                   "06h" = c("JLO", "LAV","LBO"),
                                   "12h" = c("LAX", "LBP", "JLQ"),
                                   "24h" = c("LBQ", "LAZ", "JLT"),
                                   "24hbis" = c("LI2", "LI4", "LBQ")),
                       "DON" = list("03h" = c("LCL","LCR","LCX"),
                                    "06h" = c("LDI", "LDN","LLDR"),
                                    "12h" = c("LEC", "LEI", "LEL"),
                                    "24h" = c("LB5", "LAB", "LBX")),
                       "DDG" = list("03h" = c("LCG","LCM", "LCT"),
                                    "06h" = c("LDD", "LDK","LDP"),
                                    "12h" = c("LE8", "LED", "LEJ"),
                                    "24h" = c("LB0", "JLU", "LBS")),
                       "AOA" = list("03h" = c("LCI","LCP","LCV"),
                                    "06h" = c("LDF","LDL","LDQ"),
                                    "12h" = c("LE9","LEF","LEK"),
                                    "24h" = c("LB2","JLW","LBU")),
                       "aK" = list("24h" = c("LHP", "LHR", "LHT")),
                       "VPA" = list("24h" = c("LHV", "LHX", "LHY")))
...
peak_control = dir(path = paste0(the_dir, "results/genomic_ranges/static_peaks"), pattern = "Xvivo_00h_gr.rda", full.names = TRUE)
peak_control_name = dir(path = paste0(the_dir, "results/genomic_ranges/static_peaks"), pattern = "Xvivo_00h_gr.rda")
...
for (sublist in 1:length(conditions_list)) {
  ...
  if (names(conditions_list[sublist]) != "Xvivo") { # no drawing with only Xvivo point
        ...
        names(list_bam_files) = substr(list_bam_files, nchar(list_bam_files)-18, nchar(list_bam_files)-16)
...
############# PROBLEME avec le bAM 00h => passage en bed graph
        ...
        # juste pour tester le snakefile, à enlever après
        tab = tibble(Check = character())
        tab[1,] = "Ok"
        write.table(tab, file = paste0(the_dir, "results/Gene_coverage/", names(conditions_list[sublist]), "_", df_region$gene[i], "_Coverage_plot.txt"), 
                    sep = "\t", quote = FALSE)  
