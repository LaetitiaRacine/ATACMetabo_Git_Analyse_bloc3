...
for(i in 1: length(times)){
  ...
  print(paste(nrow(peaks_df_time_intersection),"peaks detected at",time))
...
times = c("00h","05h","24h","48h")
load("/media/romuald/TOSHIBA EXT/Romuald_NGS/MARS-ATAC/data/prom_gene_fantom.rda")

prom_gene_fantom_gr = GRanges(
  seqnames = prom_gene_fantom$seqnames,
  ranges = IRanges(prom_gene_fantom$start, prom_gene_fantom$end),
  strand = "*"
  )
...
for(time in 1:length(times)){
  ...
  # First a matrix is created filled with FALSE and added to the Grange
  gr_peak = peaks_gr_time_list[[time]]

  # for each of the annotations types an overlap is calculated and used to assigned the peak as TRUE when overlapping with the annotation
    overlaps = findOverlaps(gr_peak, prom_gene_fantom_gr)
    peaks_in_prom = gr_peak[queryHits(overlaps),]
    mcols(peaks_in_prom) = mcols(prom_gene_fantom_gr)[subjectHits(overlaps),]
   
     # print(
     #  paste(
     #    nrow(unique(mcols(peaks_in_prom))),
     #    "different promoter overlapping with peaks at",
     #    times[time]
     #    )
     #  )
    ...
    print(
      paste(
        length(unique(peaks_in_prom)),
        "different peakss overlapping with a prom at",
        times[time]
        )
      )
